...
deb http://ftp.br.debian.org/debian trixie-updates main contrib non-free non-free-firmware' \
    ...
    --run-command "echo 'source /etc/network/interfaces.d/*' > /etc/network/interfaces" \
...
for i in {1..30}; do
    ...
    if [ "${i}" -eq 15 ]; then
        ...
        virsh qemu-agent-command "${VM_NAME}" '{"execute":"guest-exec","arguments":{"path":"dhclient","arg":["-v","enp2s0"],"capture-output":true}}' 2> /dev/null
